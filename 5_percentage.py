    ...
    for dy in day:
        for fn in file_name:
            ...
            if dy=='01':
                ...
                pre=fn+'_pre'
                post=fn+'_post'
                pre.append(task01.loc[index,pre])
                post.append(task01.loc[index,post])
            else:
                index=(task02.index[task02['被験者']==sm])[0]
                pre=fn+'_pre'
                post=fn+'_post'
                pre.append(task02.loc[index,pre])
                post.append(task02.loc[index,post])
            #print(index)
            #================================================
            total=0 #視線データの数
...
s2=pd.Series(pre)
print("100:",s1.corr(s2)) 
s1=pd.Series(around_150)
s2=pd.Series(pre)
print("150:",s1.corr(s2)) 
s1=pd.Series(around_200)
s2=pd.Series(pre)
print("200:",s1.corr(s2)) 
s1=pd.Series(around_250)
s2=pd.Series(pre)
print("250:",s1.corr(s2)) 
